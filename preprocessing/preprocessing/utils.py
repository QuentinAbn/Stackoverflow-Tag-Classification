...
class BaseTextCategorizationDataset:
    ...
    def to_indexes(self, labels):
        ...
        for label in labels:
            list_index.append(self.get_label_to_index_map()[label])
...
class LocalTextCategorizationDataset(BaseTextCategorizationDataset):
    ...
    def load_dataset(filename, min_samples_per_label):
        ...
        # TODO: CODE HERE
        list_columns = ['post_id', 'tag_name', 'tag_id', 'tag_position', 'title']
        dataset_columns = dataset.columns
        assert all(col in dataset_columns for col in list_columns)
        # assert list(list_columns) == list(dataset.columns)
        ...
        def filter_tags_with_less_than_x_samples(x):
            def filter_function(df):
                ...
                """
                # TODO: CODE HERE
                return df.groupby('tag_name').filter(lambda y: len(y)>=x)
            return filter_function
        ...
        # TODO: CODE HERE
        filtered_df = (dataset.pipe(filter_tag_position(0)).pipe(filter_tags_with_less_than_x_samples(min_samples_per_label)))
        return filtered_df
